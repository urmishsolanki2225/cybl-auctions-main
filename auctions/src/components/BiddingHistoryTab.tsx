...
const BiddingHistoryTab = () => {
  ...
  const filteredHistory = (status: string) =>
    biddingHistory.filter(
      (item) => status === "all" || item.bid_status === status
    );
      ...
      <div className="bidding-content">
        ...
        <div className="bidding-details">
          <div className="bid-info">
            ...
            <span className="bid-amount">
              ${parseFloat(item.highest_bid).toLocaleString()}
            </span>
          </div>
          <div className="bid-info">
            <span className="bid-label">My Bid</span>
            <span className="bid-amount my-bid">
              ${parseFloat(item.my_last_bid).toLocaleString()}
            </span>
          </div>
        </div>
        ...
        <div className="end-time">
          ...
          <span className="end-time-value">
            {new Date(item.lot_end_time).toLocaleString()}
          </span>
        </div>
        ...
        {item.bid_status === "active" && (
          <button
            ...
            onClick={() => navigate(`/lot/${item.inventory_id}`)}
          >
            Watch
          </button>
        )}
  ...
  const EmptyState = ({ status }: { status: string }) => (
    <div className="empty-state">
      ...
      <p>
        You don't have any {status === "all" ? "" : status} auction lots at the
        moment.
      </p>
    </div>
  );
